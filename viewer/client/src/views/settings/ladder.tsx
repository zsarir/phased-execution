...
function NumberField({
  id, label, hint, value, unit, zero, min = 0, step = 1, disabled, onSave,
}: {
  id: string;
  label: string;
  hint: string;
  ...
  const commit = () => {
    ...
    if (!Number.isFinite(next) || next < min) { setText(String(value)); return; }
    if (next === value) return;
    onSave(next);
  };
  const onKey = (event: KeyboardEvent<HTMLInputElement>) => {
    if (event.key === 'Enter') { (event.target as HTMLInputElement).blur(); }
    if (event.key === 'Escape') { setText(String(value)); (event.target as HTMLInputElement).blur(); }
  };
  return (
    <div className="flex flex-wrap items-center justify-between gap-2">
      <span className="min-w-0">
        <label htmlFor={id} className="text-sm text-ink">{label}</label>
        <span className="mt-0.5 block text-2xs text-ink-muted">{hint}{zero ? ` ${zero}` : ''}</span>
      </span>
    ...
    <Card>
      ...
      <CardBody className="flex flex-col gap-3">
        <p className="text-2xs text-ink-muted">
          A stopped phase is classified (never started, work in progress, done but unrecorded,
          verification red, declared blocked, a resource wall…) and the autopilot climbs that
          situation&apos;s ladder — its own session first, a fresh briefed session next — until a rung
          holds or these caps are spent. Then it leaves ONE errand and drives everything else.
        </p>

        <h3 className="text-2xs font-medium uppercase tracking-[0.14em] text-ink-faint">Caps — rungs and dollars</h3>
        <NumberField
          id="ladder-per-phase-rungs" label="Rungs per phase" value={prefs.ladderPerPhaseRungs} unit="rungs"
          hint={`How many rungs one phase may climb before its errand is written (shipped: ${LADDER_PREF_DEFAULTS.ladderPerPhaseRungs}).`}
          disabled={busy} onSave={num('ladderPerPhaseRungs')}
        />
        <NumberField
          id="ladder-per-phase-usd" label="Spend per phase" value={prefs.ladderPerPhaseUsd} unit="USD" step={10}
          hint={`What the ladder may spend on one phase, every rung counted (shipped: $${LADDER_PREF_DEFAULTS.ladderPerPhaseUsd}).`}
          disabled={busy} onSave={num('ladderPerPhaseUsd')}
        />
        <NumberField
          id="ladder-per-run-rungs" label="Rungs per run" value={prefs.ladderPerRunRungs} unit="rungs"
          hint={`Across every phase of one run (shipped: ${LADDER_PREF_DEFAULTS.ladderPerRunRungs}).`}
          disabled={busy} onSave={num('ladderPerRunRungs')}
        />
        <NumberField
          id="ladder-per-run-usd" label="Spend per run" value={prefs.ladderPerRunUsd} unit="USD" step={10}
          ...
          disabled={busy} onSave={num('ladderPerRunUsd')}
        />
        <NumberField
          id="ladder-per-day-usd" label="Spend per day" value={prefs.ladderPerDayUsd} unit="USD" step={10}
          hint={`Across every run this console drives in a day (shipped: $${LADDER_PREF_DEFAULTS.ladderPerDayUsd}).`}
          disabled={busy} onSave={num('ladderPerDayUsd')}
        />

        <h3 className="mt-1 text-2xs font-medium uppercase tracking-[0.14em] text-ink-faint">Clocks</h3>
        <NumberField
          id="ladder-converge-every" label="Sweep every" value={minutes(prefs.convergeEveryMs)} unit="min"
          ...
          disabled={busy} onSave={(v) => save.mutate({ convergeEveryMs: v * 60_000 })}
        />
        <NumberField
          id="ladder-mcp-require-timeout" label="Park on a required MCP server for" value={minutes(prefs.mcpRequireTimeoutMs)} unit="min"
          ...
          disabled={busy} onSave={(v) => save.mutate({ mcpRequireTimeoutMs: v * 60_000 })}
        />
        <NumberField
          id="ladder-budget-raise" label="Raise a spent run budget once by" value={prefs.budgetAutoRaisePct} unit="%"
          ...
          disabled={busy} onSave={num('budgetAutoRaisePct')}
        />

        <h3 className="mt-1 text-2xs font-medium uppercase tracking-[0.14em] text-ink-faint">What it may do by itself</h3>
        <div className={row}>
          <span className="min-w-0">
        ...
        <div className={row}>
          <span className="min-w-0">
            ...
            <span className="mt-0.5 block text-2xs text-ink-muted">
              An expired foreign lock over unfinished work is taken over and the work continued.
              A LIVE session&apos;s claim is never touched, whatever this says.
            </span>
          </span>
        ...
        <div className={row}>
          <span className="min-w-0">
            ...
            <span className="mt-0.5 block text-2xs text-ink-muted">
              A lane a console restart killed resumes its own session when the console is back —
              at most 3 restarts in a row per phase, then an errand.
            </span>
          </span>
        ...
        <div className={row}>
          <span className="min-w-0">
            ...
            <span className="mt-0.5 block text-2xs text-ink-muted">
              A signed-out run account, or a usage window too far out to sleep on, switches to a
              registered account that can pay — the session&apos;s transcript comes along. Off: the
              wall halts with an errand.
            </span>
          </span>
        ...
        <p className="text-2xs text-ink-muted">
          Every automatic act is journalled on the run (<code>phase.situation</code>,{' '}
          <code>phase.rung</code>, <code>phase.errand</code>, <code>run.converge</code>) and yields
          to your Stop. QA is never dispatched by itself — that stays a press. The session-presence
          hook beside this card is what lets the loop see the sessions it shares the repository with.
        </p>
      </CardBody>
    </Card>
  );
