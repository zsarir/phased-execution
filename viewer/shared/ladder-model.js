 ...
 */
export const RUNG_VEHICLES = Object.freeze(/** @type {const} */ ([
...
export function rungKey(situation, rung) {
  const params = rung.params
    ? Object.entries(rung.params).sort(([a], [b]) => a.localeCompare(b)).map(([k, v]) => `${k}=${v}`).join(',')
    : '';
...
export function untriedRungs(situationKeyOrId, history) {
  const key = situationKeyOrId in RUNGS_BY_SITUATION ? situationKeyOrId : parseSituationKey(situationKeyOrId).id;
...
export function rungLabel(vehicle, params, situation) {
  const same = (/** @type {Rung} */ rung) => rung.vehicle === vehicle
    && rungKey('x', { vehicle, params }) === rungKey('x', rung);
  const tables = situation ? [rungsFor(situation), ...Object.values(RUNGS_BY_SITUATION)] : Object.values(RUNGS_BY_SITUATION);
